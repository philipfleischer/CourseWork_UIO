...
// currently only sets up a socket for the sap.
L2SAP* l2sap_create(const char* server_ip, int server_port) {
    ...
    int rc = bind(sock, (struct sockaddr *)&addr, sizeof(struct sockaddr));
    err(rc, -1, "bind error");
 ...
 */
 ...
 uint8_t *prepend_header(const uint8_t *data, L2Header *header, int len) {
    //uint8_t *frame = malloc(sizeof(L2Header) + len);  // Bruk sizeof(L2Header) + len i stedet for header->len
    uint8_t *frame = calloc(1, sizeof(L2Header) + len);
...
L2Header *get_header(uint8_t *frame) {
    L2Header *header = malloc(sizeof(L2Header));
    memcpy(header, frame, sizeof(L2Header));

    return header;
}

void strip_header(uint8_t *frame, int len) {
    // len skal være lengden på dataene etter headeren, ikke totalrammen
...
int l2sap_sendto( L2SAP* client, const uint8_t* data, int len ) {
    ...
    if (header.len > MAX_L2_FRAMESIZE) {
        fprintf(stderr, "Payload of size <%d> exceeds limit <%d> -> dropping frame\n", 
                len, MAX_L2_FRAMESIZE - HEADER_SIZE);
        return -1;
    }

    uint8_t *frame = prepend_header(data, &header, len);
    frame[CHECKSUM_INDEX] = compute_checksum(frame, header.len);
...
int l2sap_recvfrom_timeout( L2SAP* client, uint8_t* data, int len, struct timeval* timeout ) {
    ...
    L2Header *header = get_header(buf);

    uint8_t recv_checksum = compute_checksum(buf, header->len);
    //uint8_t recv_checksum = compute_checksum(buf, rc);
    ...
    strip_header(buf, header->len - HEADER_SIZE);
    //DENNE:
    //memcpy(data, buf, len);
    //memcpy(data, buf, rc);
    int payload_len = header->len - HEADER_SIZE;
    if (payload_len > len) payload_len = len;
    memcpy(data, buf, payload_len);
    free(header);
    return rc;
}
